...
This is a single-file FastAPI microservice (`extractor/main.py`) that extracts animal/plant/fungi entity frequencies from dramatic texts for the EcoCor ecological analysis project (part of DraCoR).
...
4. Only nouns are checked; each token's lemma is matched against lowercase lemmas of entity names
...
- Entity names can have multiple word forms (e.g., hyphenated compounds); all word parts are checked against token lemmas
...
**Input/output format** is documented in `test/test.json` (input) and the Pydantic models in `extractor/main.py` (`Segment`, `NameInfo`, `NameInfoFrequency`, `ExtractorResponse`).
